...
try {
    ...
    $rules = [
        ...
        'year' => 'required|integer|minvalue:1900|maxvalue:' . date('Y'),
    ...
    if ($validator->fails()) {
        foreach ($validator->errors() as $field => $messages) {
            $errors[$field] = $messages[0]; // Get the first error message for each field
        }
        throw new Exception("Validation failed. Please correct the errors and try again.");
    }

    // echo "Validation passed! You can now process the data and save it to the database.";
